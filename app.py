...
try:
    ...
    # --- BEGIN: ปรับปรุงส่วนแจ้งเตือนให้ละเอียดที่สุด ---
    ...
    st.error(
        ...
        **ยังคงเชื่อมต่อ Google Sheet ไม่ได้**
        ...
        เราได้ตรวจสอบแล้วว่าโค้ดและ Library ทั้งหมดทำงานได้ปกติ ปัญหานี้จึงเหลือเพียง 3 สาเหตุหลักที่เกี่ยวข้องกับการตั้งค่าของคุณเท่านั้นครับ ขอให้คุณใจเย็นๆ และตรวจสอบอีกครั้งอย่างละเอียดถี่ถ้วนที่สุดครับ

        **Checklist "ตรวจสอบสามครั้ง" (สาเหตุ 99% อยู่ในนี้):**

        **1. การแชร์ชีทผิดพลาด (สาเหตุอันดับ 1):**
        - **อีเมลถูกต้องหรือไม่?** ตรวจสอบว่าอีเมลที่แชร์ให้ใน Google Sheet คืออีเมลนี้ **เป๊ะๆ** (ไม่มีการสะกดผิด):
          `{client_email}`
        - **สิทธิ์ถูกต้องหรือไม่?** ตรวจสอบว่าสิทธิ์ที่ให้คือ **"Editor"** ไม่ใช่ "Viewer" หรือ "Commenter"
        - **คุณเป็นเจ้าของชีทหรือไม่?** หากคุณไม่ใช่เจ้าของไฟล์ Google Sheet อาจมีข้อจำกัดด้านความปลอดภัยอื่นๆ ให้ลองสร้างชีทใหม่ที่คุณเป็นเจ้าของเอง แล้วคัดลอกข้อมูลไปวางเพื่อทดสอบ

        **2. ข้อมูล Secrets ผิดพลาด (สาเหตุอันดับ 2):**
        - **URL ถูกต้องหรือไม่?** ตรวจสอบว่า URL ใน Secrets ไม่มีตัวอักษรขาดหรือเกิน และตรงกับ URL ของชีทคุณจริงๆ
        - **คัดลอก `private_key` มาครบหรือไม่?** ค่า `private_key` นั้นยาวมาก ตอนคัดลอกอาจเกิดความผิดพลาดได้ง่าย ให้ลองคัดลอก-วางใหม่อีกครั้ง และตรวจสอบว่าขึ้นต้นด้วย `-----BEGIN PRIVATE KEY-----` และลงท้ายด้วย `-----END PRIVATE KEY-----`
        ...
        **3. API ยังไม่พร้อมใช้งาน:**
        ...
        - แม้จะขึ้นว่า "Enabled" บางครั้งอาจต้องใช้เวลา 5-10 นาทีเพื่อให้ระบบของ Google อัปเดตสมบูรณ์
        - ลอง **"ปิด" (Disable) API** แล้ว **"เปิด" (Enable) ใหม่อีกครั้ง** เพื่อกระตุ้นให้ระบบอัปเดต

        หลังจากตรวจสอบครบทุกข้อแล้ว ให้ **Reboot** แอปของคุณอีกครั้งครับ
        """
    )
