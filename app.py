...
try:
    ...
    # --- BEGIN: ปรับปรุงส่วนแจ้งเตือนให้ละเอียดขึ้น ---
    ...
    st.error(
        ...
        **ไม่พบ Google Sheet หรือไม่ได้รับอนุญาตให้เข้าถึง**
        ...
        ถึงแม้ว่าคุณจะแชร์ชีทให้ "หุ่นยนต์" ของเราแล้ว แต่ยังคงเกิดปัญหานี้อยู่ สาเหตุที่เป็นไปได้มากที่สุดคือ **คุณยังไม่ได้เปิดใช้งาน Google Sheets API** ครับ

        **Checklist การแก้ไข (กรุณาตรวจสอบทั้ง 3 ข้อ):**

        **1. (สำคัญที่สุด) เปิดใช้งาน Google Sheets API:**
        - **ไปที่ลิงก์นี้:** `https://console.cloud.google.com/apis/library/sheets.googleapis.com?project={project_id}`
        - หากปุ่มแสดงว่า **"Enable"** ให้กดเพื่อเปิดใช้งาน API
        - หากปุ่มแสดงว่า **"Manage"** แสดงว่า API เปิดใช้งานอยู่แล้ว ให้ไปตรวจสอบข้อต่อไป

        **2. ตรวจสอบการแชร์ชีทอีกครั้ง:**
        - **คัดลอกอีเมลนี้:** `{client_email}`
        - **ไปที่ Google Sheet ของคุณ:** [คลิกที่นี่เพื่อเปิดชีท]({sheet_url})
        - คลิกปุ่ม **"Share"** และตรวจสอบให้แน่ใจว่าอีเมลด้านบนได้รับสิทธิ์เป็น **"Editor"**

        **3. ตรวจสอบ URL ใน Secrets:**
        - ไปที่ **Settings > Secrets** ของแอปนี้ และตรวจสอบว่า URL ของ `spreadsheet` ถูกต้อง

        หลังจากตรวจสอบครบทั้ง 3 ข้อแล้ว ให้กด **Reboot** ที่เมนู 'Manage app' อีกครั้งครับ
        """
    )
